...
datapath = join(rootpath, "ALFF_data")
phenopath =join(rootpath, "pheno", "ALFF")
...
for isub in list(subjlist):
	SubALFF = '%s__ALFF.csv' % (isub)
	subfname = join(datapath, SubALFF)
	data12 = pd.read_csv(subfname)
	#drops first col (its NaN and 'ALFF')
	data12.drop(data12.columns[0], axis=1, inplace=True)
...
for col in colnumlist:
	...
	if  (Wilk_ASD.loc['Parcel_%s' % col, 'p_value'] > 0.05 and
		...
		anova_lm = ols('Parcel_%s  ~ C(Diagnosis) * C(Sex) + FIQ + MeanFD' % col, data=extended_pheno_data).fit()
